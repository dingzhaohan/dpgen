...
# ===== config ======
config_json = eval(open("./config.json").read())
base_url = config_json['host'].strip('/') + ":%s" % config_json['port'] + '/'
endpoint = config_json["endpoint"]
bucket = config_json["bucket"]
remote_oss_url = endpoint[:7] + bucket + "." + endpoint[7:].strip("/")  + "/"
...
class CloudServer:
        ...
        if job_info['type'] == 'run':
            if job_info['current_stage'] == '0':
                ...
                for ii in result:
                    ...
                    url = base_url + 'insert_lcurve_data'
    ...
    def tar_upload_submit_tasks(self, resources, command, work_path, tasks, forward_common_files, forward_task_files, backward_task_files, forward_task_dereference, job_info):
        ...
        for task in tasks:
            ...
            input_data['job_resources'] = remote_oss_url + remote_oss_dir
...
def get_job_summary(input_data):
    url = base_url + 'get_job_details?job_id=%s&username=%s' % (input_data['previous_job_id'], input_data['username'])
    time.sleep(0.2)
    return_data = []
    res = get_url(url=url)
    data = res['data']
...
def submit_job(input_data, previous_job_id=None):
    ...
    url = base_url + 'insert_job'
    time.sleep(0.2)
    res = post_url(url, json.dumps(data))
    return res['job_id']
...
def get_bucket():
    url_0 = base_url + "get_sts_token"
    oss_info = get_url(url_0)
